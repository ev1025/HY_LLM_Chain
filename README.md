...
이 저장소는 대학 입시 상담 챗봇의 **RAG 서버**입니다. 40여 개 국내 대학의 수시/정시 모집요강 PDF를 근거 데이터로 삼습니다. 질문이 들어오면 하이브리드 검색과 재순위화로 근거를 모으고, 출처가 있는 답변을 스트리밍으로 생성합니다.

대화 문맥도 함께 관리합니다. 사용자별 대화 기록은 PostgreSQL에 쌓이며, 8건 이상이 되면 LLM이 이전 대화를 요약본(사용자 프로필, 200토큰 이내)으로 압축합니다. 이후 질문부터는 이 요약본과 최근 8건의 대화가 프롬프트에 함께 제공되어, 오래된 대화의 맥락을 잃지 않으면서 토큰 사용량을 일정하게 유지합니다.

```mermaid
flowchart LR
    subgraph P1["1. 데이터 적재 (배치)"]
        A["Google Drive<br/>PDF"] --> B["파싱 · 청킹<br/>임베딩"]
        B --> C[("Elasticsearch")]
    end
    subgraph P2["2. 질의응답 (실시간)"]
        Q["사용자<br/>질문"] --> R["검색<br/>재순위화"]
        R --> G["답변<br/>스트리밍"]
        H[("PostgreSQL<br/>대화 기록")] -. "요약본 + 최근 8건" .-> G
        G -. "기록 저장" .-> H
    end
    C -. 근거 문서 .-> R
```
...
6. 답변 생성 — 검색 근거에 대화 요약본과 최근 8건 대화를 더해 프롬프트를 구성하고, 한국어 답변을 스트리밍합니다.
7. 저장 — 질문/답변을 PostgreSQL에 기록하고, 대화가 8건 이상이면 요약본을 백그라운드로 생성·갱신합니다.
...
스트리밍 챗봇의 체감 성능은 전체 응답 시간보다 **첫 토큰이 도착하기까지의 시간(TTFT)**이 좌우합니다. 그래서 Locust 시나리오(`eval/locustfile.py`)를 두 지표를 분리 측정하도록 설계했습니다.

| 측정 항목 | 방법 |
| --- | --- |
| TTFT (`stream`) | 요청 후 첫 토큰 수신까지의 지연. `chunk_size=1`로 첫 바이트를 즉시 감지 |
| E2E (`non_stream`) | 답변 스트리밍이 끝날 때까지의 전체 시간과 누적 응답 바이트 |

- 가상 사용자가 0.2~0.5초 간격으로 `/chat-stream`에 실제 입시 질문을 반복 전송합니다.
- 응답의 `Content-Type`에 따라 SSE(`text/event-stream`)와 일반 청크 스트림을 자동 분기해 동일한 지표로 기록합니다.
- 오류 발생 시점(첫 토큰 전/후)을 구분해서, 첫 토큰 실패와 중간 끊김이 각각 어느 지표에 잡히는지 명확히 했습니다.
