...
async def lifespan(app: FastAPI):
    ...
    active_backend = config.get("active_backend", "opensearch")
...
# FastAPI app & CORS
# =========================
app = FastAPI(title="KGS Opensearch RAG 서버", lifespan=lifespan)
origins = [
...
async def chat_with_rag_stream(
    ...
    async def stream_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # yield "data: [DONE]\n\n"
            # [선택] 종료 시그널이 필요하다면 보낼 수 있음 (필수 아님)
...
if __name__ == "__main__":
    uvicorn.run("open_server:app", host="0.0.0.0", port=8000, reload=True)